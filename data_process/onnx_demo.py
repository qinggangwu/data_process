...
def decode( alphabet, t, preds_value, length):
    # alphabet = "0123456789:-"
    char_list = []
    sum = 0
    ...
    if len(re) == 0:
        ...
        return re, round( sum/len(re),4)

def resize_norm_img(img, image_shape):   # image_shape = (h,w,c)
    # imgH, imgW ,imgC = image_shape
    h = image_shape[0]
    w = image_shape[1]
    ratio = w / float(h)
    imgH, imgW, imgC = 32, int(32 * ratio) ,3

    resized_image = cv2.resize(img, (int(32 * ratio), 32))
    ...
    padding_im[:, :, 0:imgW] = resized_image
    return padding_im

def chinese_demo(ALPHABETS,img_path, onnx_path ="E:/Pytorch/PaddleOCR-release-2.2/output/onnx_model/ppocr_server.onnx" ):
    session = onnxruntime.InferenceSession(onnx_path)
    input_name=getattr(session.get_inputs()[0],'name')

    img = cv2.imdecode(np.fromfile(img_path , dtype=np.uint8), -1)

    img = resize_norm_img(img, img.shape)
    preds = session.run([], {input_name: img[np.newaxis, :]})

    preds_index = np.unravel_index(np.argmax(preds[0][0], axis=1), preds[0][0].shape)[1]  # 获取每个预测位置上的最大值索引
    preds_value = np.max(preds[0][0], axis=1)  # 获取每个预测位置上的最大值
    sim_pred ,soure= decode(ALPHABETS,preds_index,preds_value, len(preds_index))

    print(sim_pred, soure)
...
def onnx_sim():
    import onnx
    file_path = 'D:/model/paddleocr/onnx_model/tune.onnx'
    ...
    # print(model.graph.input[0]);quit()
    model.graph.input[0].name = 'input'
    model.graph.output[-1].type.tensor_type.shape.dim[0].dim_param = '1'    # 320%4 =80
    model.graph.output[-1].type.tensor_type.shape.dim[1].dim_param = '?'    # 320%4 =80
    model.graph[-1].name = 'out'


    # onnx.checker.check_model(onnx_model)
    # onnx.save(onnx_model, onnxfile)

    onnx.save(model, 'D:/model/paddleocr/onnx_model/tunes.onnx')
...
if __name__ == '__main__':
    onnx_path = "D:/model/paddleocr/onnx_model/ppocr_server_sims.onnx"
    onnx_path = "D:/model/paddleocr/onnx_model/tune3.onnx"
    # img_path = 'E:/Pytorch/PaddleOCR-release-2.2/doc/imgs_words/ch/word_5.jpg'
    img_path = 'D:/Backup/桌面/cut2.jpg'

    info = open('E:/Pytorch/PaddleOCR-release-2.2/ppocr/utils/tune.txt', encoding='utf-8').readlines()
    ALPHABETS = "".join([a[:-1] for a in info])
    # t = time.time()
    # # ALPHABETS = '0123456789.-#{}<>= '
    chinese_demo(ALPHABETS ,img_path,onnx_path)
    # print(time.time() -t)
    # onnx_sim()