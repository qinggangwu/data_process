...
def gnt2npy(src_file, dst_file, image_size, map_file):
    ...
    if os.path.exists(map_file):
        with open(map_file, 'r',encoding='unicode_escape') as fp:
            for line in fp.readlines():
                print(line)
                if len(line) == 0:
                    continue;
                code, label = line.split()
                code_map[int(code)] = label
        fp.close()
    images = []
    labels = []
...
if __name__=='__main__':
    ...
    # map_file = sys.argv[4]
    # src_file = 'D:/data/中文手写数据/字符样本数据/Gnt1.0TrainPart2.zip'
    # dst_file = 'D:/data/中文手写数据/字符样本数据/mpy/Gnt1.0TrainPart2'
    src_file = 'D:/data/中文手写数据/字符样本数据/Gnt1.0Test'
    dst_file = 'D:/data/中文手写数据/字符样本数据/mpy/test'
    image_size = 32
    # map_file = 'D:/data/中文手写数据/map_file.txt'
    map_file = 'D:/data/中文手写数据/map_dict.txt'
    gnt2npy(src_file, dst_file, image_size, map_file)
    # gnt2npy(src_file, dst_file, 32, map_file)
